...
import sys, numpy, random, matplotlib.pyplot as plt, setInput, pyNN.spiNNaker as sim
...
sim.setup(timestep=dt, min_delay=1.0, max_delay=10.0, threads=8)  # delays in ms
...
cellType = sim.IF_curr_exp(**cellParams)
"""
connections = {
    # Input and LGN
...
network = buildNetworkAndConnections(ImageNumPixelRows, ImageNumPixelColumns, numOrientations, oriFilterSize, V1PoolSize, V2PoolSize, phi,
    ...
    for i in range(ImageNumPixelRows):         # Rows
        for j in range(ImageNumPixelColumns):  # Columns
            ...
            if inputPoisson:
                ...
                rate_bright = 10.0 * max(0.0, input[i][j]/127.0 - 1.0)
                rate_dark   = 10.0 * max(0.0, 1.0 - input[i][j]/127.0)
                ...
                LGNBright[i*ImageNumPixelColumns + j].set('i_offset', rate_bright)
                LGNDark  [i*ImageNumPixelColumns + j].set('i_offset', rate_dark)
