...
@nrp.MapSpikeSource("LGNBrightInput", nrp.map_neurons(range(0, 5*5), lambda i: nrp.brain.LGNBright[i]), nrp.dc_source)
@nrp.MapSpikeSource("LGNDarkInput",   nrp.map_neurons(range(0, 5*5), lambda i: nrp.brain.LGNDark[i]),   nrp.dc_source)
@nrp.Robot2Neuron()
...
@nrp.MapSpikeSink("V4", nrp.map_neurons(range((3-1)*5*5), lambda: nrp.brain.V4Brightness[i]), nrp.leaky_integrator_alpha)
...
@nrp.MapRobotSubscriber("camera", Topic("/icub_model/left_eye_camera/image_raw", sensor_msgs.msg.Image))
@nrp.MapSpikeSource("SurfaceSegmentationOffSignal", nrp.map_neurons(range((3-1)*5*5), lambda i: nrp.brain.SurfaceSegmentationOff[i]), nrp.dc_source)
@nrp.MapSpikeSource("SurfaceSegmentationOnSignal",  nrp.map_neurons(range((3-1)*5*5), lambda i: nrp.brain.SurfaceSegmentationOn [i]), nrp.dc_source)
@nrp.MapSpikeSource("BoundarySegmentationSignal",   nrp.map_neurons(range((3-1)*5*5), lambda i: nrp.brain.BoundarySegmentationOn[i]), nrp.dc_source)
@nrp.Robot2Neuron()
...
# Output transfer function to display V1 layer 23 activity density map [NEED TO CHANGE ALL VARIABLE TO NRP.BRAIN.(...)]
@nrp.MapCSVRecorder("csv_recorder", filename="V1_activity.csv", headers=map(str, range(2*6*6)))
...
@nrp.MapSpikeSink("V1LayerToPlot", nrp.map_neurons(range(2*6*6), lambda i: nrp.brain.V1Layer23[i]), nrp.spike_recorder)
@nrp.Neuron2Robot()
def plot_V1_Activity(t, cumplotDensityV1, V1LayerToPlot, csv_recorder):
    ...
    V1SpikeCountUpToNow = [value for (key, value) in sorted(V1LayerToPlot.get_spike_counts().items())]
    ...
    for k in range(numOrientations):                    # Orientations
        for i in range(numPixelRows):                   # Rows
            for j in range(numPixelColumns):            # Columns
                plotDensityV1[k][i][j] = plotDensityV1[k][i][j] + V1SpikeCountUpToNow[k*numPixelRows*numPixelColumns + i*numPixelColumns + j] - cumplotDensityV1[k][i][j]
                cumplotDensityV1[k][i][j] = cumplotDensityV1[k][i][j] + plotDensityV1[k][i][j]  # update cumulative spike count
    ...
    for i in range(numPixelRows):         # Rows
        for j in range(numPixelColumns):  # Columns
            if numOrientations==2:          # Vertical and horizontal
                dataV1[i][j] = [plotDensityV1[0][i][j], plotDensityV1[1][i][j], 0]
...
# Output transfer function to display LGN activity density map [NEED TO CHANGE ALL VARIABLE TO NRP.BRAIN.(...)]
@nrp.MapCSVRecorder("csv_recorder", filename="LGN_activity.csv", headers=map(str, range(5*5)))
@nrp.MapVariable("cumplotDensityLGN", scope=nrp.GLOBAL, initial_value=[[0 for col in range(5)] for row in range(5)])
@nrp.MapSpikeSink("LGNLayerToPlot", nrp.map_neurons(range(5*5), lambda i: nrp.brain.LGNBright[i]), nrp.spike_recorder)
@nrp.Neuron2Robot()
def plot_LGN_Activity(t, cumplotDensityLGN, csv_recorder, LGNLayerToPlot):

    nRows = 5
    nCols = 5

    # clientLogger.info(str(cumplotDensityLGN.value[0][0]))

    # Record spike count up to now from LGN
    plotDensityLGN = [[0 for col in range(nCols)] for row in range(nRows)]
    LGNSpikeCountUpToNow = [0 for slot in range(nRows*nCols)]
    # LGNSpikeCountUpToNow = [value for (key, value) in sorted(LGNLayerToPlot.get_spike_counts().items())]

    # LGN sampling and update of the cumulative spike count
    for i in range(nRows):
        for j in range(nCols):
            plotDensityLGN[i][j] = plotDensityLGN[i][j] + LGNSpikeCountUpToNow[i*nCols + j] - cumplotDensityLGN.value[i][j]
            cumplotDensityLGN.value[i][j] = cumplotDensityLGN.value[i][j] + plotDensityLGN[i][j]  # update cumulative spike count

    # Publish the V1 activity density map
    csv_recorder.record_entry(*plotDensityLGN)